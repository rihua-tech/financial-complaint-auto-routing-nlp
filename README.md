...
2. **Data ingestion**: Download separate 2024 and 2025 CFPB complaint datasets locally through the reusable data-ingestion workflow.
...
The raw CSV files are downloaded locally by `notebooks/01_data_download.ipynb`. They are not included in this repository and should not be uploaded to GitHub. The notebook defaults to `FORCE_DOWNLOAD = False`, so Run All validates existing local ignored CSVs when they are present and only calls the CFPB API when a raw CSV is missing or `FORCE_DOWNLOAD = True`.
...
Within each month, the notebook allocates the monthly target across daily windows as evenly as possible. This reduces both late-year recency bias and within-month end-of-month bias. The notebook keeps only rows with non-empty complaint narratives, non-empty product labels, unique `complaint_id` values, and `date_received` values within the target year. It preserves the original CFPB API columns in each raw local CSV for traceability.
